...
The cache directory needs to be in a web accessible location, so config/environment.rb needs something like this:
...
You need some apache config along the lines of this (note that <code>/radiant-cache</code> here matches <code>/public/radiant-cache</code> in "Radiant Setup" above - if these don't agree Radiant will write a cache, but Apache will not use it):
  ...
  RewriteRule ^(/)?$ /radiant-cache/index.cache.html [P,QSA,L]
  ...
  # Rewrite /path/to/page to cache/path/to/page.cache.html if it exists
  RewriteCond %{DOCUMENT_ROOT}/radiant-cache%{REQUEST_URI}.cache.html -f
  RewriteRule . /radiant-cache%{REQUEST_URI}.cache.html [P,QSA,L]
  
  # Rewrite /path/to/page/ (trailing slash) to cache/path/to/page.cache.html if it exists
  RewriteCond %{REQUEST_URI} ^(.+)/$
  RewriteCond %{DOCUMENT_ROOT}/radiant-cache$1.cache.html -f
  RewriteRule ^(.+)/$ /radiant-cache$1.cache.html [P,QSA,L]
