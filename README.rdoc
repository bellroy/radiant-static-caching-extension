...
a cache, but Apache will not use it):
  ...
  RewriteRule ^/?$ /radiant-cache/index.html [proxy,last]
  ...
  # Rewrite /path/to/page/ (trailing slash) to cache/path/to/page if it exists (for paths already
  # containing an extension or no extension)
  RewriteCond %{REQUEST_METHOD} ^(GET|HEAD)$
  RewriteCond %{QUERY_STRING} ^$
  RewriteCond %{REQUEST_URI} ^(.+)/$
  RewriteCond %{DOCUMENT_ROOT}/radiant-cache%1 -f  # %1 is match from last RewriteCond
  RewriteRule ^(.+)/$ /radiant-cache$1 [P,L]
  ...
  RewriteRule ^(.+)/?$ /radiant-cache$1.%1 [P,L]  # %1 is match from last RewriteCond
